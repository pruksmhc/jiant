...
def load_tsv(
    ...
    if s2_idx is not None:
        mask = mask & (rows[s2_idx].str.len() > 0) & (rows[s2_idx] != 'none')
    if s3_idx is not None:
        mask = mask & (rows[s3_idx].str.len() > 0) & (rows[s3_idx] != 'none')
    if has_labels:
    ...
    if return_indices:
        ...
        return sent1s.tolist(), sent2s.tolist(), sent3s.tolist(), labels.tolist(), idxs
    elif tag2idx_dict is not None:
        return sent1s.tolist(), sent2s.tolist(), sent3s.tolist(), labels.tolist(), tagids
    else:
        return sent1s.tolist(), sent2s.tolist(), sent3s.tolist(), labels.tolist()
