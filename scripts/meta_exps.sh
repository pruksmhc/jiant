...
# debug
python main.py --config config/meta.conf --overrides "exp_name = metalearn, run_name = debug-meta, max_vals = 3, val_interval = 100, do_train = 1, train_for_eval = 0, train_tasks = \"mnli,snli\", eval_tasks = \"none\", do_eval = 0, metatrain = 0, slow_params_approx = 1, sent_enc = conv, d_hid = 512, mnli_pair_attn = 0, snli_pair_attn = 0, pseudo_meta = 1"
...
## FEEL THE METALEARN ##
#python main.py --config config/meta.conf --overrides "train_tasks = \"mnli,snli\", eval_tasks = \"mnli,snli\", run_name = exact-mnli-snli-schdcos-r2, slow_params_approx = 0, mnli_pair_attn = 0, snli_pair_attn = 0, lr = .00003, sim_lr = .00001, random_seed = 222, scheduler = cosine" 
