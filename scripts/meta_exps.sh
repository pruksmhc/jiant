...
# just Wiki103 [will break b/c metatrainer expects two tasks
#python main.py --config config/meta.conf --overrides "train_tasks = \"wiki103\", eval_tasks = \"wiki103\", run_name = exact-wiki103-s2, slow_params_approx = 0, batch_size = 8, lr = .00003, sim_lr = .00003, cuda = ${gpuid}, random_seed = ${seed}, d_hid = 256, sent_enc = convlm , conv_layers = \"${DAUPHIN_WIKI103_CONVS}\", multistep_loss = 1, multistep_scale = 0.01" 
...
# just Wiki103
#python main.py --config config/meta.conf --overrides "train_tasks = \"wiki103\", eval_tasks = \"wiki103\", run_name = approx-wiki103-s2, slow_params_approx = 1, batch_size = 8, lr = .00003, sim_lr = .00003, cuda = ${gpuid}, random_seed = ${seed}, d_hid = 256, sent_enc = convlm , conv_layers = \"${DAUPHIN_WIKI103_CONVS}\", multistep_loss = 1, multistep_scale = 0.01" 
